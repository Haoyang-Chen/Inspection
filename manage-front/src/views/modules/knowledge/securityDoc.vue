  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item label="时效性" prop="status">
        <el-select
          ...
          clearable
          >
          <el-option v-for="item in secDocStatusOptions" :key="item.label" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="24" class="mb8">
      <el-col class="table-title-panel">
        <span class="table-title">安全文件列表</span>
    ...
    >
      ...
      <el-table-column label="文件编号" align="center" prop="no" :show-overflow-tooltip="true" />
      <el-table-column label="文件名称" align="center" prop="title" :show-overflow-tooltip="true" />
    ...
    <!-- 添加或修改参数配置对话框 -->
    <securityDoc-dialog ref="securityDocDialog" v-model="open" :editable="editable" @submitSuccess="getList"></securityDoc-dialog>
    <!-- 符合性评价 -->
...
<script>
import { listSecdoc, delSecdoc } from "@/api/modules/knowledge/securityDoc";
import SecurityDocDialog from "@/views/components/knowledge/securityDoc";
...
export default {
  ...
  computed: {
    ...
    /** 时效性数据字典 */
    secDocStatusOptions() {
      return this.$store.state.common.secDocStatus;
    }
  },
  created() {
    this.getList();
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSecdoc(this.queryParams).then(response => {
    ...
    /** 根据时效性value获取label */
    statusName(val) {
      return this.secDocStatusOptions.find(item => item.value === val).label;
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.open = true;
    ...
    handleDelete(row) {
      ...
      this.rowsOpRequest(delSecdoc, idList, "安全文件").then(_ => {
        this.getList();
      });
    },
    /** 搜索按钮操作 */
    ...
    handleEdit(row, editable) {
      ...
      this.$nextTick(() => {
        this.$refs.securityDocDialog.setup(row.id);
      })
    },
    /** 符合性评价 */
